package com.lifesense.healthcenter.service.wechat.token;
...
import com.lifesense.healthcenter.service.wechat.token.dto.TokenDto;
...
public class TokenService {
	...
	public static int checkErrcode(JSONObject json) {
		...
		if (0 == errcode || 43004 == errcode) { // 已发送成功，说明43004表示需要接收者关注（即用户已取消关注）
			return 0;
		} else if (42001 == errcode || 40001 == errcode || 40002 == errcode
				|| 40014 == errcode) { // accessToken超时或失效后重发3次
			return 1;
		} else if(45015 == errcode){// 回复时间超过限制(客服接口，用户与公众号超48小时没有交互)
			return 3;
		} else { // 未知错误
			return 2;
		}
	...
	public static String updateAccessToken() {
		...
		try {
			...
			String accessToken = StringUtil.toString(tockenMap
					.get(ACCESS_TOKEN));
	...
	public static String getAccessToken() {
		...
		if (diff > tokenDto.getExpiresIn() - 2500) {
			return updateAccessToken();
		}
	...
	private static Map<String, Object> getAccessTokenByWeixin(Integer queryTimes) {
		...
		// 调用接口获取access_token
		String body = HttpClientManager.get(MessageFormat.format(
				WeixinAPI.TOKEN_URL, appid, appSecret));
